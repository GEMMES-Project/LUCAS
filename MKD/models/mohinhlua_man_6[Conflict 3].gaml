...
/** Insert the global definitions, variables and actions here */
	...
	file tramquantrac_file <- shape_file("../includes/dulieuthaydoi/input_mohinh/tramquantracman_point.shp");
...
//			write tramquantr;
		...
		ask vungthuyloi
		{
			...
			cells<- (cell_lua where (each.risk = -1.0)) where (each intersects self.shape);
			ask cells
			{
				risk <- myself.risk;
			}
...
//			}
		...
		ask vungthuyloi{
			if(current_date.month <6 and length(domanmax)>0){
				float tmp<-domanmax[current_date.month];
				ask cells{
...
grid cell_lua files: [lua_file,lua_file_validate] use_individual_shapes: false use_regular_agents: false neighbors: 8
{
	float damage <- 0.0;
	float risk <- -1.0;
	float priority_salt <- 0.0;
...
//
//	reflex ss {
////		if ((cell_lua at int(self)).grid_value != grid_value){
...
/** Insert here the definition of the input and output of the model */
	output
	{
		display "Hien trang lua 2016" refresh:every(1#cycle)//type: opengl
...
//			species river aspect: default;
		...
		display "compare" refresh:every(1#cycle){
