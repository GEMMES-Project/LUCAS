...
//	file lua_file_validate <- grid_file("../includes/dulieuthaydoi/input_mohinh/lua_muakho2016_kiemchung_asc.asc");
	file lua_file_validate <- grid_file("../includes/dulieuthaydoi/input_mohinh/bandokiemchung_2016_asc.asc");
	...
	file biendong_file <- shape_file("../includes/caclopphu/bienDong_region.shp");
	file tieuvungthuyloi_file <- shape_file("../includes/caclopphu/tieuvungthuyloi.shp");
	file khungbd_file <- shape_file("../includes/caclopphu/khungbando.shp");
...
//	file vungthuyloi_file <- shape_file("../includes/dulieuthaydoi/input_mohinh/vungthuyloi_risk.shp");
	file vungthuyloi_file <- shape_file("../includes/dulieuthaydoi/input_mohinh/vungthuyloi_st_chianho_region.shp");
	...
	float damage_adjust <- 0.1; //0.1
	float damage_threshold <- 0.7; //0.4
	...
	init
	{
		...
		create tieuvungthuyloi from: tieuvungthuyloi_file;
		create biendong from: biendong_file;
		...
		//			write "" + tramquantr + " " + domanmax;
			...
			float the_area<-self.shape.area / 0.5e9; 
			ask cells
			{
				risk <- myself.risk;
			...
			ask cells
			{
				dd <- dd / mm *(flip(1/the_area)?0.3:0.01);
			}
	...
	reflex end_simulation when: current_date.year = 2016 and current_date.month = 5 and current_date.day >= 14
	{
		...
		if (length(cell_lua where (each.dead)) > 0)
		{
			...
			write "Dien tich dat lua:" + length(cell_lua); 
		}
...
/** Insert here the definition of the input and output of the model */
	output
	{
		...
		display "So sanh sai lech" refresh: every(1 # day) type: opengl
		{
...
//	parameter salinity_threshold var: salinity_threshold min: 4.0 max: 28.0 step: 4.0;
	parameter damage_adjust var: damage_adjust min: 0.1 max: 1.0 step: 0.1;
	parameter damage_threshold var: damage_threshold min: 0.1 max: 1.0 step: 0.1;
	parameter risk_threshold var: risk_threshold min: 1.0 max: 10.0 step: 1.0;

	//	method exhaustive minimize: objective ;
	method genetic pop_dim: 3 crossover_prob: 0.7 mutation_prob: 0.1 improve_sol: true stochastic_sel: false nb_prelim_gen: 1 max_gen: 5 minimize: objective aggregation: "min";
