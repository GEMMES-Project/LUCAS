...
model BDIdecision
import "common_model_scena_1.gaml"
...
global {
	...
	file land_unit2010_file<-file("../includes/environmental/land_unit2010.shp");
	file land_unit2020_file<-file("../includes/environmental/land_unit2010.shp");	// land_unit of the senario , This file are set = landunit 2010 by default, 
...
//	modelID <-"BDI";
	...
	action create_parcel {
		...
		create FarmerBDI  from: land_parcel_file with: [region::int(read('Region')),landuse_init::string(read('Lu10')),landuse_obs::string(read('Lu10')), land_unit::int(read("Landunit")), nearhouse::int(read("Nearhouse")),acid_sulfat::string(read("Acid_sul_d")), parcel_area::read("Area")]
		{
		...
		create landunit_parcel from: land_unit2010_file with: [landunit_id::string(read('landunit'))]; 		
...
//		get land_unit id from land unit agent to the parcel
		loop lunit_obj  over: landunit_parcel{									//loop for the land unit object  - declared in the common model  
			ask land_parcel overlapping lunit_obj{								// select the land parcel object that overlap with land unit object
...
//				\write ("plan change neighbors; money:" + money +"; income="+ income);
	...
	bool have_loan_success<-false;
	plan loan_from_banks 
		intention:request_invesment_from_bank
		finished_when:have_loan_success{
			if(debug){write "loan";}
			have_loan_success<-false;
//			if(flip( profiles[profile] *100)){
				income<-income+1000;
				have_loan_success<-true;
				do add_desire(earn_the_highest_possible_income);
				do add_belief(request_invesment_from_bank);  // remove the disire 
//			}
	}
...
//	high high land suitability
		...
		loop new_landuse over:(LS_choice){						
			...
			if(flip(profiles[profile]*2) and new_landuse="TSL"){
...
//			do remove_belief(earn_the_highest_possible_income);
	...
	float updatePriority (string lu)
	{
		float pp <- (known_profit(lu) /max_price * weight_profit + compute_cost(lu) * weight_cost+ compute_implementation(lu) * weight_implementation ) / (weight_profit + weight_implementation + weight_cost);
		return pp;
	}
	
	float known_profit(string lu) {
		if (lu in price_map.keys) {
//			return float(get_belief_with_name("profit,"+ lu + "," + land_unit).value);
		}
		return 0.0;
	}
...
//				write LS_map[lu];
		...
		if (lu in suitability_map[land_unit].keys) {
			sss<- suitability_map[land_unit][lu]<sss?suitability_map[land_unit][lu]:sss;
		}
		return sss;
	}
