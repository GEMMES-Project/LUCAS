...
global {
	...
	file land_unit_file<-file("../includes/LU_Myxuyen2005/landuse_myxuyen_2005_region.shp");
...
//	modelID <-"BDI";
	...
	action create_parcel {
		...
		create FarmerBDI  from: land_parcel_file with: [region::int(read('Region')),landuse_init::string(read('lu2005')),landuse_obs::string(read('Lu10')),  nearhouse::int(read("Nearhouse")),acid_sulfat::string(read("Acid_sul_d")), parcel_area::read("Area"),land_unit::int(read("Landunit"))]
		//
		{
			//temp
			land_unit<-1+rnd(5);
...
//		create landunit_parcel from: land_unit_file with: [landunit_id::int(read('LANDUNIT'))]; 		
		create landunit_parcel from: land_unit_file with: [landunit_id::int(read('lu2005'))]; 		
