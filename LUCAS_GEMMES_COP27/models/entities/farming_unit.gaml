...
grid farming_unit file: cell_file neighbors: 8 schedules: [] use_individual_shapes: false use_regular_agents: false use_neighbors_cache: false {
	...
	action luachonksd {
		...
		if (my_province != nil) {
			my_province.debt <- my_province.debt + debt / 1E7;
			my_province.benefit <- my_province.benefit + benefit / 10000;
		}

		if (my_aez != nil) {
			my_aez.debt <- my_aez.debt + debt / 1E7;
			my_aez.benefit <- my_aez.benefit + benefit / 10000;
		}
